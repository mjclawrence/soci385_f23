---
title: "Continuing With Confidence Intervals"
...
```{r}
#| label: calculate standard error of age
...
```{r}
#| label: table of offspring variable
...
addmargins(table(cupid$kids, exclude = NULL))
```
...
```{r}
#| label: confidence interval without kids
...
```{r}
#| label: confidence interval with kids
...
```{r}
#| label: confidence intervals with group_by and summarise

cupid |> 
  filter(!is.na(kids)) |>
  group_by(kids) |> # for each value of kids variable (0/1)
...
```{r}
#| label: prop table for orientation

addmargins(round(prop.table(table(cupid$orientation, exclude = NULL)),3))
```
...
```{r}
#| label: group by with more than one variable
...
```{r}
#| label: mean of binary variable is the proportion with a 1!
mean(cupid$gay)
```
...
```{r}
#| label: lower and upper limits for proportion confidence interval
