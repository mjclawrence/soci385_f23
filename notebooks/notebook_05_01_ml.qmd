...
```{r}
#| label: cor racial segregation and income

cor(cz$hh_income, cz$racial_seg, 
    use = "complete")
```
...
```{r}
#| label: cor racial segregation and income by urban

cor(cz$racial_seg[cz$urban==0], 
    cz$hh_income[cz$urban==0], 
    use = "complete")

cor(cz$racial_seg[cz$urban==1], 
    cz$hh_income[cz$urban==1], 
    use = "complete")
```
...
urban_rural_plot <- ggplot(cz, aes(x = hh_income, 
                                          y = racial_seg,
                                          color = urban,
                                          shape = urban))

# The `shape = ` option makes points different shapes for each value
...
```{r}
#| label: cor eviction and vaccination
...
Could do a column plot with the summarized correlation coefficients...

```{r}
#| label: column plot example
...
evictions |> 
  ...
  summarise(cor = cor(evict_pct, vax_pct,
                      use = "complete")) |> 
  ggplot(aes(x = reorder(xsite, cor), y = cor)) +
  geom_col() +
  theme(axis.text.x = element_text(size = 5,
                                   angle = 45))
```
Example of how to reorder columns by the values of a different variable

```{r}
#| label: reordered column plot example
...
evictions |> 
  ...
  summarise(cor = cor(evict_pct, vax_pct,
                      use = "complete")) |> 
  ggplot(aes(x = reorder(xsite, cor), # order xsite by min-max values of cor 
             y = cor)) +
  geom_col() +
  theme(axis.text.x = element_text(size = 7)) # change size of x axis text labels
```

Could have a scatterplot with different colored points for each city...

```{r}
#| label: scatterplots with city colors
...
Could use facet grid...

```{r}
#| label: facet_grid example
...
evictions |> 
  ...
  ggplot(aes(x = evict_pct, y = vax_pct,
             color = xsite)) +
  geom_point(size = .5) + facet_wrap(~xsite, scales = "free") +
  guides(color = "none") +
  labs(x = "Eviction Rate \n(number of evictions by number of households)",
       y = "Vaccination Rate") +
...
```{r}
#| label: more options for facet_wrap figures
...
evictions |> 
  ggplot(aes(x = evict_pct, y = vax_pct,
             color = majority_race,
             group = xsite)) +
  geom_point(size = .75) + facet_wrap(~xsite, scales = "free") +
  theme(legend.position = "bottom") + geom_smooth(method = "lm", se = FALSE)
```
...
evictions |> 
  ggplot(aes(x = evict_pct, y = vax_pct,
             color = xsite)) +
  geom_point() + facet_wrap(~xsite, scales = "free") +
  guides(color = "none") + geom_smooth(method = "lm", se = FALSE) +
  scale_color_viridis_d(option = "turbo") # Palette Name
```