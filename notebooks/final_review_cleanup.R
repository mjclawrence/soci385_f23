...
tiger <- tiger |> 
  ...
  mutate(crimsent = factor(crimsent, 
                           levels = c("Too little time", "About the right amount", "Too much time"))) |> 
  mutate(
    value = factor(value, 
  ...
  mainPanel(
    tabsetPanel(
      ...
      tabPanel("Descriptive Plot", 
               helpText("These plots show how the distribution of beliefs on length of prison sentencing varies by characteristic and beliefs on the function of cancel culture."),
               plotOutput("descriptive_plot")),
      tabPanel("Hypothesis Test", 
               helpText("Significance of Difference between Opinions on Cancel Culture and Opinions on Time Spent in Prison"),
...
server <- function(input, output) {
  ...
  output$descriptive_plot <- renderPlot({
    plot1 <- tiger_filter() |> 
      ...
      labs(x ="Opinion on Cancel Culture", y="Distribution of Responses", fill = "Opinion of Crime Sentencing") +
  ...
  output$hypothesis_table <- renderDataTable({
    table1 <- tiger_filter() |> 
      ...
      datatable(rownames = FALSE,
                caption = "Highlighted groups indicate a significant association between beliefs about informal and formal punishment",
                options = list(
                  dom = 'Bt', # see explanation below
  ...
  output$cancul_descriptives <- renderTable({
    tiger_filter() |> 
      mutate(cancul1 = factor(cancul1,
             labels = c("Hold people accountable for their actions", "Punish people who didn't deserve it"))) |> 
